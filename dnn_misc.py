...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        N = X.shape[0]
        forward_output = np.dot(X, self.params['W']) + np.tile(self.params['b'], (N,1))
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(np.ones(N).reshape(1, N), grad)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        # PLEASE follow the Heaviside step function defined in CSCI567_HW2.pdf                                                   #
        ##########################################################################################################################
        backward_output = grad * np.minimum(1, np.ceil(np.maximum(0, X)))
        return backward_output
